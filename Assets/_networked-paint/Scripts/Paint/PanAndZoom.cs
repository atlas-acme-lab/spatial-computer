...
public class PanAndZoom : MonoBehaviour
{
    ...
    [Header("Canvas")]
    public PaintableCanvas Canvas;

    // [Header("OSC")]
    // public SendPaintOSC OSCSender;
    ...
    void UpdateWithTouch()
    {
        ...
        if (touchCount == 2)
        {
            ...
            // Debug.Log(Mathf.Abs(currentDistance - previousDistance).ToString());

            if ((Mathf.Abs(currentDistance - previousDistance) > 10))
            {
                    ...
                    case TouchPhase.Moved:
                        {
                            ...
                            if (touch0.deltaPosition != Vector2.zero && isTouching)
                            {
                                OnSwipe(touch0.deltaPosition);
                            }
                            break;
                        }
                    case TouchPhase.Ended:
                        {
                            if (Time.time - touch0StartTime <= maxDurationForTap
                                && Vector2.Distance(touch0.position, touch0StartPosition) <= maxDistanceForTap
                                && isTouching)
                            {
                                OnClick(touch0.position);
                            }

                            if (onEndTouch != null) onEndTouch(touch0.position);
                            isTouching = false;
                            cameraControlEnabled = true;
                            break;
                        }
    ...
    void OnSwipe(Vector2 deltaPosition)
    {
        ...
        if (controlCamera && cameraControlEnabled)
        {
            ...
            // OSCSender.SendCameraPos(cam.transform.position);
    ...
    void OnPinch(Vector2 center, float oldDistance, float newDistance, Vector2 touchDelta)
    {
        ...
        if (controlCamera && cameraControlEnabled)
        {
            ...
            if (cam.orthographic)
            {
                ...
                // OSCSender.SendCameraZoom(cam.orthographicSize);
